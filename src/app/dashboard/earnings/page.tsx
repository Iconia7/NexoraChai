...
export default function EarningsPage() {
    ...
    if (loading) return <div className="min-h-screen flex items-center justify-center bg-brand-beige-light font-black uppercase tracking-widest text-brand-muted">Loading Earnings...</div>;
            ...
            <div className="flex-1 flex flex-col min-w-0">
                ...
                <main className="flex-1 p-8 overflow-y-auto">
                    ...
                    <header className="flex justify-between items-center mb-10">
                        <div>
                            <h1 className="text-3xl font-black tracking-tight mb-2">Earnings</h1>
                            <p className="text-brand-muted font-medium">Manage your wallet and withdraw your creator funds.</p>
                        </div>
                        <button
                            onClick={handleExportCSV}
                            className="bg-white px-5 py-3 rounded-2xl border border-black/5 flex items-center gap-2 text-xs font-black uppercase tracking-widest card-shadow hover:scale-[1.02] transition-transform"
                        >
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-12">
                            ...
                            <div className="relative z-10">
                                <div className="flex justify-between items-start mb-12">
                                    <div>
                                        ...
                                        <h2 className="text-6xl font-black tracking-tight">KES {(data.wallet?.balance || 0).toLocaleString()}.00</h2>
                                    </div>
                                ...
                                <div className="flex flex-col md:flex-row gap-6 items-stretch md:items-center">
                                    <button
                                        ...
                                        className="bg-[#00E676] text-black px-10 py-5 rounded-2xl font-black text-sm uppercase tracking-widest flex items-center justify-center gap-2 hover:bg-[#00C853] transition-all hover:scale-[1.02] shadow-xl shadow-[#00E676]/20"
                                    >
                                        Withdraw to M-Pesa <ArrowUpRight size={18} />
                                    </button>
                                    <Link href="/dashboard/settings" className="bg-white/10 text-white px-10 py-5 rounded-2xl font-black text-sm uppercase tracking-widest hover:bg-white/20 transition-all border border-white/10 backdrop-blur-md flex items-center justify-center gap-2">
                        ...
                        <div className="bg-white rounded-[3rem] p-10 card-shadow border border-black/[0.02] flex flex-col justify-between">
                            <div>
                                ...
                                <p className="text-[10px] font-black text-brand-muted uppercase tracking-widest mb-2">International (Paystack)</p>
                                <h3 className="text-4xl font-black tracking-tight mb-4">KES {(data.paystackTotal || 0).toLocaleString()}.00</h3>
                            ...
                            <button
                                ...
                                className="w-full mt-8 flex items-center justify-between text-brand-primary font-black uppercase tracking-widest text-xs group"
                            >
                            ...
                            {data.transactions.map((t: any) => (
                                <div key={t.id} className="flex items-center justify-between p-4 hover:bg-brand-beige-light/50 rounded-2xl transition-colors border border-transparent hover:border-black/5">
                                    ...
                                    <div className="text-right">
                                        <p className={`font-black tracking-tight text-lg ${t.status === 'COMPLETED' ? (t.type === 'WITHDRAWAL' ? 'text-red-500' : 'text-brand-secondary') : 'text-brand-muted opacity-40'}`}>
                                            {t.type === 'WITHDRAWAL' ? '-' : '+'} KES {t.netAmount.toLocaleString()}
                                        </p>
                                        <div className="flex items-center justify-end gap-2">
                                            {t.status !== 'COMPLETED' && (
                                                <span className={`text-[8px] font-black uppercase tracking-tighter px-1.5 py-0.5 rounded border ${t.status === 'FAILED' ? 'border-red-200 text-red-400 bg-red-50' : 'border-amber-200 text-amber-500 bg-amber-50'}`}>
                                                    {t.status}
                                                </span>
                                            )}
